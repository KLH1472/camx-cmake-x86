...
public:
    ...
    static VariantGroup GetVariantGroup(const CHAR* pVariantName);
    static VariantType GetVariantType(const CHAR* pVariantName);
    static CDKResult FreeUsecaseDescriptor(ChiUsecase* pUsecase);
    static ChiUsecase* CloneUsecase(const ChiUsecase* pSrcUsecase, UINT32 numDesc, UINT32* pDescIndex);
    static ChiTarget* ClonePipelineDesc(const ChiTarget* pSrcTarget);
    static CDKResult ClonePipelineDesc(const ChiPipelineTargetCreateDescriptor* pSrcDesc,
        ChiPipelineTargetCreateDescriptor* pDstDesc);
    static VOID DestroyUsecase(ChiUsecase* pUsecase);
    static VOID DestroyPipelineDesc(ChiTarget* pTarget);
    static VOID DestroyPipelineDesc(ChiPipelineTargetCreateDescriptor* pDesc);
