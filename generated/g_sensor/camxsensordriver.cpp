...
///
/// Auto-generated by: Parameter Parser V2.0.0 (1910091319)
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    : ParameterModule(name, VersionUnion(MAJOR, MINOR, PATCH), 0, ModeUnion(0))
{
    ...
    PARAMETER_INITIALIZE(delayInfoCount);
    PARAMETER_INITIALIZE(delayInfoID);
    ...
    UINT64 alignment)
{
    ...
    // Reading Array mipiFlags from symbol table
    {
        result = result && entry->Read(&structure->mipiFlagsCount, alignment);
        structure->mipiFlagsID = entry->ID;
        ParameterFileSymbolTableEntry* pointerEntry = entry->Table->ReadPointerEntry(entry, alignment);
        result = result && pointerEntry != NULL;
        if (result)
        {
            structure->mipiFlags = PARAMETER_NEW MipiFlagsType[structure->mipiFlagsCount];
        }
        if (!result || structure->mipiFlags == NULL)
        {
            structure->mipiFlagsCount = 0;
        }
        for (UINT32 i = 0; result && i < structure->mipiFlagsCount; i++)
        {
            structure->mipiFlags[i] = (MipiFlagsType)pointerEntry->ReadEnum(&result, alignment);
        }
    }
    ...
    UINT64 alignment)
{
    ...
    // Reading Optional parameter delayType from symbol table
    {
        result = entry->Read(&structure->delayTypeExists, alignment);
        structure->delayTypeID = entry->ID;
        ParameterFileSymbolTableEntry* pointerEntry = entry->Table->ReadPointerEntry(entry, alignment);
        result = result && pointerEntry != NULL;
        if (result && structure->delayTypeExists)
        {
            structure->delayType = (DelayInfoType)pointerEntry->ReadEnum(&result, alignment);
        }
        else
        {
            PARAMETER_INITIALIZE(structure->delayType);
        }
    }

    // Reading Optional parameter linecount from symbol table
    {
        result = result && entry->Read(&structure->linecountExists, alignment);
        structure->linecountID = entry->ID;
        ParameterFileSymbolTableEntry* pointerEntry = entry->Table->ReadPointerEntry(entry, alignment);
        result = result && pointerEntry != NULL;
        if (result && structure->linecountExists)
        {
            result = result && pointerEntry->Read(&structure->linecount, alignment);
        }
        else
        {
    ...
    UINT64 alignment)
{
    ...
    // Reading Array delayInfo from symbol table
    {
        result = result && entry->Read(&structure->delayInfoCount, alignment);
        structure->delayInfoID = entry->ID;
        ParameterFileSymbolTableEntry* pointerEntry = entry->Table->ReadPointerEntry(entry, alignment);
        result = result && pointerEntry != NULL;
        if (result)
        {
            structure->delayInfo = PARAMETER_NEW DelayInfo[structure->delayInfoCount];
        }
        if (!result || structure->delayInfo == NULL)
        {
            structure->delayInfoCount = 0;
        }
        for (UINT32 i = 0; result && i < structure->delayInfoCount; i++)
        {
            result = LoadDelayInfo(pointerEntry, &structure->delayInfo[i], alignment);
        }
    }
    ...
    ResolutionData* structure)
{
    ...
    PARAMETER_DELETE[] structure->mipiFlags;
}
    ...
    SensorDriverData* structure)
{
    ...
    PARAMETER_DELETE[] structure->delayInfo;
}
