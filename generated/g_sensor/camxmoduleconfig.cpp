    ...
    UINT64 alignment)
{
    ...
    // Reading Optional parameter i2cFrequencyMode from symbol table
    {
        result = result && entry->Read(&structure->i2cFrequencyModeExists, alignment);
        structure->i2cFrequencyModeID = entry->ID;
        ParameterFileSymbolTableEntry* pointerEntry = entry->Table->ReadPointerEntry(entry, alignment);
        result = result && pointerEntry != NULL;
        if (result && structure->i2cFrequencyModeExists)
        {
            structure->i2cFrequencyMode = (I2CFrequencyMode)pointerEntry->ReadEnum(&result, alignment);
        }
        else
        {
            PARAMETER_INITIALIZE(structure->i2cFrequencyMode);
        }
    }
